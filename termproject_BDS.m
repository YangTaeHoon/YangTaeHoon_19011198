...
%%% 시작시간 %%%%
starttime = datetime(2023, 2, 26, 0, 0, 0);

%%% 다음날 자정 설정 %%%
nextday = starttime + days(1);
...
%%% 매개변수 %%%
load('nav.mat');
a = nav.BDS.a/1000; % Semi-major-axis [km]
e = nav.BDS.e; %eccentricity
i = rad2deg(nav.BDS.i); %inclination [deg]
w = rad2deg(nav.BDS.omega); %argument of perigee [deg]
M0 = nav.BDS.M0; %Mean Anomaly [rad]
toc = nav.BDS.toc; %특정 시간
W = rad2deg(nav.BDS.OMEGA); %Right ascension of the ascending node [deg]
