...
%%% 시작시간 %%%%
starttime = datetime(2023, 4, 29, 0, 0, 0);
...
%%% 매개변수 %%%
load('nav.mat');
a = nav.GPS.a/1000; % Semi-major-axis [km]
e = nav.GPS.e; %eccentricity
i = rad2deg(nav.GPS.i); %inclination [deg]
w = rad2deg(nav.GPS.omega); %argument of perigee [deg]
M0 = nav.GPS.M0; %Mean Anomaly [rad]
toc = nav.GPS.toc; %특정 시간
W = rad2deg(nav.GPS.OMEGA); %Right ascension of the ascending node [deg]
